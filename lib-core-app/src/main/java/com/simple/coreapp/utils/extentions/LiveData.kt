...
inline fun <T> LiveData<T>.observeLaunch(owner: LifecycleOwner, crossinline action: suspend (T) -> Unit) {
...
inline fun <T> LiveData<T>.observeQueue(owner: LifecycleOwner, tag: String = "ui", crossinline action: suspend (T) -> Unit) {

    observe(owner) {

        JobQueueManager.submit(tag, owner.lifecycleScope.coroutineContext) {
            action.invoke(it)
        }
    }
}
