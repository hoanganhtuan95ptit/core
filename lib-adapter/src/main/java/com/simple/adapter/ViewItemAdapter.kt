...
abstract class ViewItemAdapter<out VI : ViewItemCloneable, out VB : ViewBinding>(
    ...
    val viewItemClass: Class<ViewItemCloneable> by lazy {

        this.findGenericClassBySuperClass(ViewItemCloneable::class.java)!!.java
    }
    ...
    open fun onBindViewHolder(holder: BaseBindingViewHolder<ViewBinding>, binding: @UnsafeVariance VB, position: Int) {
        ...
        binding.root.setOnClickListener { view ->
            ...
            getViewItem(holder.bindingAdapterPosition)?.let { onItemClick.invoke(view, it) }
        }
    }
...
private fun <VB : ViewBinding> Class<ViewBinding>.findBinding(parent: ViewGroup): VB {

    return findBinding(LayoutInflater.from(parent.context), parent) as VB
}


private fun <VB : ViewBinding> Class<ViewBinding>.findBinding(inflater: LayoutInflater, container: ViewGroup? = null, attackToParent: Boolean = false): VB {
